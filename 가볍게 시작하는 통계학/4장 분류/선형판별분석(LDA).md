...
위 자료에서 연체를 한 사람(333명) 중 252명을 잘못 불류하였다. 전체 오차율은 낮지만 연체한 사람들 중의 오류율은 $252 / 333 \approx 75.7$%로 상당히 높음을 알 수 있다. 이는 LDA는 전체 오차율을 낮추는 것에 초점을 두지만 개별 클래스의 오차율에는 관심이 없기 때문이다.

위와 같은 현상은 관측치를 클래스로 할당하는 임계치를 조절함으로써 해결할 수 있다. 
예를 들어 위 예시에 대해서 연체의 사후확률이 20%를 초과하는 사람은 모두 연체 클
