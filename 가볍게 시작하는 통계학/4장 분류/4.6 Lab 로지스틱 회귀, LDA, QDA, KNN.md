...
1. "Prior probabilities of groups" 즉, LDA 분석 이전 확률은 Down Up 각각 0.49와 0.5로 산정됨을 알 수 있다. 
2. "Group mean" 은 집단 별 독립변수 X의 평균을 의미한다. 즉, 각 클래스 내 각 설명변수의 평균이며 LDA는 이것을 $\mu_k$
3. "Coefficients of linear discriminants" 은 LDA분석으로 계산된 판별함수식의 계수이다.
	2개 이상의 설명변수를 가지는 LDA식은 $\delta_k(x)=x^T \boldsymbol{\Sigma}^{-1} \mu_k-\frac{1}{2} \mu_k^T \boldsymbol{\Sigma}^{-1} \mu_k+\log \pi_k$ 와 같으니
	#질문  "Coefficients of linear discriminants" 이게 무엇을 의미하는 건지 잘 모르겠음.
4. 판별함수의 상수항은 apply() 함수을 이용하여 LDA 판별함수식의 계수에 평균값을 대입하면 산정할 수 있으며 그 결과, LD1 은 -2.132868, LD2는 6.635509로 산정되었습니다.