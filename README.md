...
```
한 줄(row)에 2개 이상의 이미지를 넣고 싶다면 이미지를 표 안에 넣으면 된다.

|![[Image1|Size]]|![[Image2|Size]]| 
```
|![[로지스틱 함수 그래프.png|300]]|![[로지스틱 함수 이미지1.png|300]]|
...
### 충돌 발생시
git pull 을 입력하고 충돌이 발생하였다고 하면 git pull 명령어를 다시 입력해본다. 그럼 충돌 보고서가 생성되면서 어떤 파일에서 충돌이 일어났는지 알려준다. 옵시디언에서 해당 파일을 열어 수정해도 좋지만 VSCode에서 수정하는 것이 편리하다. 

충돌사항을 모두 정리한 뒤 변경 사항 업로드 과정을 반복하여 핸들링 된 결과를 업로드해주자.
